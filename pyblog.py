            ...
            for line in md_text:
                if "Title: " in line:
                    ...
                    if file_date == "":
                        ...
                        file_date = parse_json_config(config, "date_format").replace("YYYY", str(file_cdate.year)).replace("MM", str(file_cdate.month).zfill(2)).replace("DD", str(file_cdate.day).zfill(2))
                        file_year = str(file_cdate.year)
                    if file_author == "":
                        file_author = parse_json_config(config, "default_author")

                years.add(file_year)
                year_position = parse_json_config(config, "date_format").find("Y")
                month_position = parse_json_config(config, "date_format").find("M")
                day_position = parse_json_config(config, "date_format").find("D")
